...
> **完整缓存方案已单独成文:`docs/plans/webdav-cache.md`**(边听边缓存 + 后续章节预取、全局 LRU 淘汰、多书切换推演、设置项与测试计划),本节仅保留要点。
...
- **边听边预取**:当前章节播放时后台顺序预取后续章节(默认 5 章,可调);优先级"播放 > 预取 > 扫描",播放器一缓冲预取就让路,不影响流畅度。多书共用缓存按全局 LRU 淘汰:切书时新书的写入自然挤掉旧书"已听完的旧章节",旧书进度在数据库,回头继续听只差 1~2 秒重缓冲。
