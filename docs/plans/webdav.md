...
> 状态:**已实现**(2026-09-18,等待真机构建验证;沙箱无 Android SDK,未编译)
...
> **完整缓存方案已单独成文:`docs/plans/webdav-cache.md`**(填满式预取——未听章节提前缓存至上限、分级 LRU 淘汰防预取污染、多书切换推演、设置项与测试计划),本节仅保留要点。

- **播放缓存**:media3 `SimpleCache` + `CacheDataSource` 叠在 `AuthDataSource` 外层,存 `cacheDir`(系统空间紧张时可回收),**LRU 上限默认 1GB,设置里可调(关/512M/1G/2G/4G),可一键清除**。收益:重听/回跳秒开、切回上一章不再重新下载、弱网体验明显变好。缓存只加速,不改变"在线"本质,不会悄悄吃满存储。
- **填满式预取**:当前章节播放时,后台按顺序把**未听章节持续预取进缓存,直到整本剩余缓存完或达到上限**;优先级"播放 > 预取 > 扫描",播放器一缓冲预取就让路,不影响流畅度。多书共用缓存按**分级 LRU** 淘汰:非当前书的"预取未听数据"最先让路(防预取污染),当前书与已听数据按最近使用排序——切书时新书顺畅接管缓存,旧书进度在数据库,回头继续听只差 1~2 秒重缓冲。
